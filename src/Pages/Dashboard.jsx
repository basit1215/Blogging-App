...
const Dashboard = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (file[0]) {
      // Upload the blog image to Firebase
    ...
    await sendData(blogData, 'blogs');
    fetchBlogs(); // Refresh the list of blogs after submitting
  };
  ...
  return (
    <div className="bg-[#b8dfff] py-16  px-4 sm:px-6 lg:px-8  ">
      <h1 className='text-center font-bold text-3xl pb-5 text-blue-700 underline'>Publish New Blog</h1>
      ...
      <h1 className='text-center font-bold text-3xl py-5 text-blue-700 underline'>Your Blogs</h1>
      {loading ? (
        <p>Loading...</p>
      ) : blogs.length > 0 ? (
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2  lg:grid-cols-2 xl:grid-cols-3 gap-6">
